...
def main():  # main program loop
    ...
    channel = 'kevinshangman-' + simpledialog.askstring(
        'Input', 'channel to join'
    )

    connect(channel, local_user, on_network_message)
...
def on_network_message(timestamp, user, message):
    ...
    if user == 'system' and 'created the channel' in message:
        channel_creator = message.split("'")[1]
        if channel_creator == local_user:
            # i am the server
            print("I AM DA SERVER I WILL NOW CHOOSE A WORD!")
